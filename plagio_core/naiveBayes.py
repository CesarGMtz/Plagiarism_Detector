...
import os
csv_path = os.path.join(os.path.dirname(__file__), "Feature_Vector_CSV.csv")
...
def detectar_plagio(ruta_carpeta):
    archivos = [os.path.join(ruta_carpeta, f) for f in os.listdir(ruta_carpeta) if f.endswith('.py')]
    resultados = []
    for i in range(len(archivos)):
        for j in range(i + 1, len(archivos)):
            archivo1 = archivos[i]
            archivo2 = archivos[j]
            # Usa aquí tu modelo y extracción de features
            prob, tipo = predecir_plagio(archivo1, archivo2)  # tú defines esta función
            resultados.append((archivo1, archivo2, prob, tipo))
    return resultados
